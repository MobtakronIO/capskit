...
## Tasks
- [x] Define the `ActionInterceptor` type in `src/types.ts`.
- [x] Implement `addInterceptor` and the recursive execution chain in `src/kernel/platform.ts`.
- [x] Add a `metrics` counting interceptor or a simple logging interceptor in `verify.test.ts` to prove it wraps calls correctly.
